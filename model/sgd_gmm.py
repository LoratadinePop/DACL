...
class SGDGMM(ABC):
    ...
    def fit(self, local_rank, data, val_data=None, verbose=True, interval=1):
        ...
        # BUG: DDP training / world_size? Should the reg loss consider batch_size of the whole dataset?
        ...
        if dist.get_rank() == 0:
            ...
            # Save model on CPU
            torch.save(
                self.module.module.cpu().state_dict(),
                "./result/checkpoint/sgdgmm/gmm.pth",
            )
