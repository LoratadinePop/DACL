...
class SGDGMMModule(nn.Module):
    ...
    def predict(self, input):
        ...
        log_prob_batch = mvn(loc=self.means, scale_tril=self.L).log_prob(
            data[:, None, :])  # 512,10
        component = torch.max(log_prob_batch, 1)[1]
...
class BaseSGDGMM(ABC):
    ...
    def __init__(self, components, dimensions, epochs=100, lr=1e-3,
                 batch_size=64, tol=1e-6, restarts=1, max_no_improvement=20,
                 k_means_factor=1, w=1e-6, k_means_iters=2, lr_step=5,
                 lr_gamma=0.1, device=None, model=None):
        self.k = components
        self.d = dimensions
        self.epochs = epochs
        ...
        self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimiser, milestones=[lr_step, lr_step + 5],
                                                              gamma=lr_gamma)
    ...
    def fit(self, data, val_data=None, verbose=True, interval=1):
        ...
        init_loader = data_utils.DataLoader(data, batch_size=self.batch_size * self.k_means_factor,
                                            num_workers=0,
                                            shuffle=True,
                                            pin_memory=False,
                                            drop_last=True)
